...
list_halfdays = find_halfdays(1000, 1500);
...
list_working = setdiff(list_working, list_halfdays); %removing halfdays
...
%For testing purposes, we use 21 march as testing date
test_work_single = list_working(2);
train_work_single = sort(setdiff(list_working, test_work_single));
...
nn_params(xtrain_w_singleg, ytrain_w_single, xtest_w_singleg, ytest_w_single);

% net = nn_train(xtrain_w_singleg, ytrain_w_single, 20);
% yf = net(xtest_w_singleg')';
% disp(calculate_mape(ytest_w_single, yf));
% 
% figure();
% plot_forecasted_power(ytest_w_single, yf, test_work_single);
